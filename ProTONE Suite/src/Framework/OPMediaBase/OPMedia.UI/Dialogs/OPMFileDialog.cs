...
namespace OPMedia.UI.Controls.Dialogs
{
    ...
    public partial class OPMFileDialog : ToolForm
    {
                    ...
                    if (saveDlg != null)
                    {
                        ...
                        if (string.IsNullOrEmpty(ext) && saveDlg.AddExtension)
                        {
                            if (!string.IsNullOrEmpty(saveDlg.DefaultExt))
                                fileName = $"{fileName}.{saveDlg.DefaultExt}";
                        }
                    }
                    ...
                    foreach (string fav in favorites)
                    {
                        try
                        {
                            ...
                            if (PathUtils.IsRootPath(fav))
                            {
                                ...
                                title = $"{di.VolumeLabel} [{di.DriveType}]";
                            }
                            else
        ...
        private bool CanCloseOnEnter()
        {
            ...
            {
                MessageDisplay.Show(ex.Message, Translator.TranslatedAppName, MessageBoxIcon.Warning);
            }
            catch { }
        ...
        {
            return (MessageDisplay.Query(Translator.Translate("TXT_CONFIRM_OVERWRITE", p), Translator.TranslatedAppName) == DialogResult.Yes);
        }

        private bool ConfirmCreate(string p)
        {
            return (MessageDisplay.Query(Translator.Translate("TXT_CONFIRM_CREATE", p), Translator.TranslatedAppName) == DialogResult.Yes);
        }
