...
namespace OPMedia.UI.HelpSupport
{
    public class HelpViewer : ToolForm
    {
        ...
        void OnHelpDocumentLoadCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            {
                docToDisplay = GenerateDocument($"Exception occured: {e.Error}");
            }
            else
            {
            ...
            if (uri.Scheme == "file")
            {
                try
                {
                    ...
                    for (int i = 0; i < docLines.Length; i++)
                    {
                        ...
                        {
                            string sReplaceWith = $"src=\"{AppConfig.InstallationPath}\\docs\\images";
                            docLines[i] = docLines[i].ToLowerInvariant().Replace("src=\"images", sReplaceWith);
                        }
                ...
                using (HttpWebResponse response = (HttpWebResponse)request.GetResponse())
                {
                    using (StreamReader sr = new StreamReader(response.GetResponseStream()))
                    {
                        ...
                        for (int i = 0; i < docLines.Length; i++)
                        {
                            ...
                            {
                                string replaceWith = $"src=\"{AppConfig.HelpUriBase}/images";
                                docLines[i] = docLines[i].ToLowerInvariant().Replace("src=\"images", replaceWith);
                            }
        ...
        {
            string newTitle = $"{Translator.TranslatedAppName} {Translator.Translate("TXT_HELP")}: {wbHelpDisplay.DocumentTitle}";
        ...
        private void UpdatePrevnextDocuments()
        {
            if (bckUrls.Count > 0)
            {
                ...
                tsbPrev.ToolTipText = $"{Translator.Translate("TXT_BACK")}: {bckUrls.Peek()}";
            }
            else
            {
            ...
            if (fwdUrls.Count > 0)
            {
                ...
                tsbNext.ToolTipText = $"{Translator.Translate("TXT_FORWARD")}: {fwdUrls.Peek()}";
            }
            else
            {
